...
- 已存在任何待决策时，公开的 `step()` 必须拒绝无响应推进；界面同时禁用
  “推进一步”，避免 OCGCore 读取默认或残留返回缓冲区。
...
- `MSG_SELECT_CHAIN` 若明确为“非强制且候选数为 0”，则不存在用户可选动作；
  C++ 可按上游协议自动提交 `int32(-1)` 跳过该窗口。只要存在候选项或为
  强制连锁，仍必须返回 `unsupported`，不得自动代替玩家选择。
